...
mod tests {
    ...
    pub fn test_link_creation() {
        let link: Link = Link::new(0, LinkType::ETHERNET, 10.0, 5.0);

        assert_eq!(link.id, 0);
        assert_eq!(link.link_type, LinkType::ETHERNET);
        assert_eq!(link.capacity, 10.0);
        assert_eq!(link.cost, 5.0)
    }

    #[test]
    pub fn test_link_comparison() {
        let first_link: Link = Link::new(0, LinkType::ETHERNET, 10.0, 5.0);
        let second_link: Link = Link::new(1, LinkType::WIRELESS, 15.0, 6.0);
        ...
        assert!(first_link.cost < second_link.cost)
    }
}
